...
export class ShellScriptDetector {
    public static readonly MAX_SHEBANG_FILE_SIZE: number = 4096;

    private static readonly shellInterpreters: Set<string> = new Set([
        "bash",
        "dash",
        "fish",
        "ksh",
     ...
     */
    public static async getInterpreter(filePath: string): Promise<string | undefined> {
        try {
            const fileStats = await stat(filePath);
            if (!fileStats.isFile() || fileStats.size > ShellScriptDetector.MAX_SHEBANG_FILE_SIZE) {
                return undefined;
            }
            ...
            if (
                !interpreter ||
                !ShellScriptDetector.shellInterpreters.has(basename(interpreter).toLowerCase())
            ) {
                return undefined;
            }
